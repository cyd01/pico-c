...
void MD5Cal( char * inString, char * outString ) {
	MD5_CTX mdContext;
	unsigned int len = strlen (inString), i ;

	MD5Init (&mdContext);
	MD5Update (&mdContext, (unsigned char*)inString, len);
	MD5Final (&mdContext);

	for (i = 0; i < 16; i++) sprintf( (outString+(2*i)), "%02x", mdContext.digest[i] ) ;
	outString[32] = '\0' ;
	}
	
void MD5CalFile (char * filename, char * outString ) {
  FILE *inFile = NULL ;
  MD5_CTX mdContext;
  int bytes,i;
  unsigned char data[1024];

	if( !strcmp(filename,"-") ) { inFile = stdin ; }
	else { inFile = fopen (filename, "rb"); }
